...
extension FlutterPluginRegistrar {
    func lookupUrl(forAsset asset: String) -> URL? {
        ...
        let url = flutterBundle.url(forResource: asset, withExtension: nil, subdirectory: "flutter_assets")

        if url == nil {
            assertionFailure("Could not get URL for asset: \(asset)")
        }
...
public class FlutterFontRegistry {
        ...
        debug: Bool = false) -> NSFont?
    {
        ...
        let size = size ?? NSFont.systemFontSize
        let defaultFont = NSFont.systemFont(ofSize: size)
        let inputs = "family: \(String(describing: family)), size: \(size), weight: \(weight), isItalic: \(isItalic)"

        guard let family = family else {
            if debug {
                self.debug("No family provided. Resolved NSFont: \(defaultFont) of size: \(size) with inputs: \(inputs)")
            }
            return defaultFont
        }
        ...
        if
            let uiFont = NSFont(name: font.name, size: size)
        {
            if debug {
                self.debug("Resolved font: \(font), NSFont: \(uiFont) from inputs: \(inputs)")
            }
            return uiFont
        }

        assertionFailure("Could not resolve NSFont from inputs: \(inputs). This is programmer error.")
        return nil
    }
    ...
    private static func loadFontManifest() -> [FontManifestEntry]? {
        ...
        guard
            let manifestUrl = registrar.lookupUrl(forAsset: "FontManifest.json"),
    ...
    private static func registerFontAsset(family: String, fontAsset: FontManifestEntry.Asset) {
        ...
        guard
            let fontUrl = registrar.lookupUrl(forAsset: fontAsset.asset)
        else {
            debug("Could not get URL for font \(fontBaseFileName)")
            return
        }
        ...
        /// The next check whether its `NSFont` can be created is the "real" test of success.
        #if DEBUG
            if
                !didRegister,
                ...
                let error = CTFontManagerError(rawValue: CFErrorGetCode(unretainedValue))
            {
                if error == .alreadyRegistered {
                    debug("Font: \(fontName), family: \(family) is already registered in CTFontManager. Attempting to get NSFont and register...")
                } else {
                    debug("An error other than CTFontManagerError.alreadyRegistered occurred attempting to register font: \(fontName), family: \(family) with asset path: \(fontAsset.asset): \(error) \(unretainedValue). Attempting to get NSFont anyway...")
                }
            }
        #endif
        ...
        guard
            ...
            debug("Could not get NSFont for cgFont: \(cgFont) family: \(family) with asset path: \(fontAsset.asset), aborting!")
            return
        }
